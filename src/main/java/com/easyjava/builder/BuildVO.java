...
public class BuildVO {
    private static final Logger logger = LoggerFactory.getLogger(BuildVO.class);
    /**
    ...
    public static void execute(TableInfo tableInfo){
        //创建存放dto象的包
        File folder = new File(Constants.VO_PATH);
        if(!folder.exists()){
            folder.mkdirs();
        }
        //创建该表的dto类
        File file = new File(folder,tableInfo.getBeanName()+"VO.java");
        ...
        try(OutputStream ops = new FileOutputStream(file);
            ...
            //指定该类所在的包
            bf.write("package "+Constants.BASE_PACKAGE+"."+Constants.POJO_PACKAGE+"."+Constants.VO_PACKAGE+";");
            bf.newLine();
            bf.newLine();
            //导入可序列化类
            ...
            bf.newLine();
            //类声明
            bf.write("public class "+tableInfo.getBeanName()+"VO implements Serializable {");
            bf.newLine();
            //属性声明
