...
public class BuildDTO {
    ...
    public static void execute(TableInfo tableInfo){
        //创建存放dto象的包
        File folder = new File(Constants.DTO_PATH);
        if(!folder.exists()){
            folder.mkdirs();
        }
        //创建该表的dto类
        File file = new File(folder,tableInfo.getBeanName()+"DTO.java");
        ...
        try(OutputStream ops = new FileOutputStream(file);
            ...
            //指定该类所在的包
            bf.write("package "+Constants.BASE_PACKAGE+"."+Constants.POJO_PACKAGE+"."+Constants.DTO_PACKAGE+";");
            bf.newLine();
            bf.newLine();
            //导入可序列化类
            ...
            bf.newLine();
            //类声明
            bf.write("public class "+tableInfo.getBeanName()+"DTO implements Serializable {");
            bf.newLine();
            //属性声明
